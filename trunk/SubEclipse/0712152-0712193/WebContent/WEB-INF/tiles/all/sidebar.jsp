...
<%
	...
	String globalcounter = "TongSoLuotTruyCap_"
			+ request.getAttribute("maGianHang");

	if (application.getAttribute(globalcounter) != null) {
		soLuotTruyCap = (Integer) application
				.getAttribute(globalcounter);
		soLuotTruyCap++;
	}
...
<div id="search_section"><s:form action="store" method="get">
	<s:hidden name="maGianHang" value="%{maGianHang}"></s:hidden>
	<s:hidden name="module" value="search"></s:hidden>
	<s:select name="loai" tooltip="Chọn loại sản phẩm cần tìm"
		list="#{'':'Tất cả','DIENTHOAI':'Điện thoại', 'LAPTOP':'Laptop'}"
		label="Loại sản phẩm" labelposition="top"></s:select>
	<s:textfield name="ten" tooltip="Nhập tên sản phẩm cần tìm"
		label="Tên sản phẩm" labelposition="top"></s:textfield>
	<s:textfield name="min" label="Giá từ" labelposition="top"></s:textfield>
...
<ul>
	<s:iterator value="dsDanhMucGianHang">
		<s:url id="danhsach" action="store">
			...
			<s:param name="maDanhMuc" value="danhMuc.maDanhMuc"></s:param>
		</s:url>
		<li><a href="${danhsach }"> <s:property value="danhMuc.tenDanhMuc" /> </a></li>
	</s:iterator>
...
<table>
	<tbody>
		...
		<tr>
			<td><b>Vùng:</b></td>
			<td><s:property value="tinhThanhPho.tenTinhThanhPho" /></td>
		</tr>
